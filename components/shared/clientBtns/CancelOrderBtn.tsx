...
// import { IOrder, IOrderDoc } from "@/models/order.model";
...
//   open: boolean;
//   // UI-only: caller can hook this to actually perform cancellation.
...
//          console.log(json,"json data here")
//          setOrder(json.order);
...
//   const cancellableUntilDate = null;
//   const now = new Date();
//   const isPastCutoff = false;
//   const cutoffText = cancellableUntilDate
//     ? `Cancellations allowed until some date`
//     : "Cancellations may require contacting support";
...
//   const handleOrderCancelation = async(orderId:string)=> {
//       try {
//          const {error,success} = await cancelOrderAction({
//            orderId,
//            reason: ""
//          })
//          if(error) {
...
//             return
//          }else if(success) {
//             setSuccessMessage("your order has been cancelled , you'll receive a cancellation order confirmation via email soon.")
...
//       <div className="relative w-full overflow-y-scroll h-full  max-w-5xl bg-white rounded-2xl shadow-2xl overflow-x-hidden">
//         {/* Header */}
//         <div className="flex items-center justify-between px-6 py-4 border-b">
//           <div>
//             <h3 className="text-lg font-semibold">Cancel Order</h3>
//             <p className="text-xs text-gray-500">Order #{order?._id} • Placed {new Date(order?.createdAt).toLocaleString()}</p>
//           </div>
//           <div className="flex items-center gap-2">
//             <button
//               aria-label="Close"
//               onClick={close}
//               className="text-sm text-gray-600 hover:text-gray-900 rounded-md px-2 py-1"
//             >
//               Close
//             </button>
//           </div>
...
//               <div className={`px-3 py-1 rounded-md ${step === 2 ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-700"}`}>2</div>
//               <div className="ml-3 text-sm text-gray-500">{step === 1 ? "Choose reason" : step === 2 ? "Review & confirm" : "Done"}</div>
//             </div>
...
//                   {reasons.map((r) => (
//                     <label key={r.id} className={`p-3 border rounded-lg cursor-pointer flex items-start gap-3 ${selectedReason === r.id ? "border-indigo-600 ring-1 ring-indigo-100 bg-indigo-50" : "border-gray-200"}`}>
//                       <input
//                         type="radio"
//                         name="cancel-reason"
//                         value={r.id}
...
//                     <textarea value={customNote} onChange={(e) => setCustomNote(e.target.value)} rows={3} className="mt-2 w-full border rounded-md p-2 text-sm" placeholder="Short note (why you're cancelling)"></textarea>
//                   </div>
//                 )}
...
//                   <div className="text-sm text-gray-600 mt-1">{cutoffText}</div>
...
//                   <button onClick={handleContinueStep1} className="px-4 py-2 bg-indigo-600 text-white rounded-md disabled:opacity-60">
//                     Continue
//                   </button>
...
//                     <p className="text-sm text-gray-700">your order has been cancelled , you'll receive a cancellation order confirmation via email soon.</p>
//                     <div className="mt-4 flex justify-center gap-3">
//                       <button onClick={() => { close(); }} className="px-4 py-2 bg-indigo-600 text-white rounded-md">Close</button>
//                       <button onClick={() => { /* optionally view orders */ }} className="px-4 py-2 border rounded-md">View orders</button>
//                     </div>
//                   </>
...
//           {/* Right column: quick order summary & policy */}
//           <aside className="lg:col-span-1 border rounded-lg p-4 bg-gray-50">
//             <div className="text-sm font-medium">Order summary</div>
//             <div className="mt-3">
//               <div className="text-xs text-gray-600">Items</div>
//               <ul className="mt-2 space-y-2">
//                 {orderSummary?.items?.slice(0, 3).map((it,index) => (
//                   <li key={index} className="flex items-center justify-between text-sm">
//                     <div className="flex items-center gap-3">
//                       <img src={it.thumbnail ?? ""} alt="" className="w-10 h-10 object-contain rounded" />
//                       <div>
//                         <div className="font-medium">{it.name}</div>
//                         <div className="text-xs text-gray-500">Qty: {it.quantity}</div>
//                       </div>
//                     </div>
//                     <div className="font-medium">${(it.unitPrice * it.quantity)?.toFixed(2)}</div>
//                   </li>
//                 ))}
//               </ul>
...
//                 <div className="font-semibold">${orderSummary?.total?.toFixed(2)}</div>
//               </div>

//               <div className="mt-3 text-xs text-gray-600">
//                 {isPastCutoff ? (
//                   <div className="text-red-600">This order is past the cancellation cutoff and may require contacting support.</div>
//                 ) : (
//                   <div>{cutoffText}</div>
//                 )}
//               </div>
...
//   );
// }
// components/shared/CancelOrderUI.tsx
"use client";
