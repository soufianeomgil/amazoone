...
const MainCard: React.FC<Props> = ({
  ...
  const slideTo = (next: number) =>
    setImgIndex(clamp(next, 0, gallery.length - 1));
  ...
  const handleCopyToAnother = async () => {
    toast.message("Copy to another wishlist (TODO)");
    setMoreOpen(false);
  };
  ...
  return (
    <div>
      ...
      >
        {/* Image area */}
        <div
          className="relative bg-gray-50 overflow-hidden flex items-center justify-center p-3 md:p-4"
          style={{ height: 165 }}
        >
              ...
              <>
                <button
                  ...
                  opacity-0 group-hover:opacity-100 transition ${
                    imgIndex === 0 ? "cursor-not-allowed opacity-40!" : ""
                  }`}
        ...
        <div className="p-3 flex-1 flex flex-col">
           <div className="mb-1">
             <span className="text-[10px] font-bold text-[hsl(178,100%,34%)] uppercase tracking-widest">
               {product.brand}
             </span>
           </div>
          {/* <h3
          ...
          >
           ...
           <Link href={`/product/${product._id}`} className="text-sm font-bold text-gray-800 line-clamp-2 hover:text-orange-600 transition-colors h-10 mb-2">
             {product.name}
           </Link>
          ...
          <div className="relative overflow-hidden h-6">
              ...
              {[...SAMPLE_CONVERSION_BADGES, ...SAMPLE_CONVERSION_BADGES].map(
                (badge, index) => (
                  <p
                    ...
                    className="text-[10px] font-semibold text-orange-600 h-6 flex items-center italic"
                  >
                    {badge.label}
                  </p>
                )
           ...
           {/* <div className="h-5 overflow-hidden mt-1">
             <div className="animate-badge-loop flex flex-col">
               {SAMPLE_CONVERSION_BADGES.map((b, i) => (
                 <span key={i} className="text-[10px] font-semibold text-orange-600 h-6 flex items-center italic">
                    {b.label}
                 </span>
               ))}
             </div>
      ...
      {isWishlist && (
        <div className="w-full mt-2 gap-2.5 flex items-center">
          <Button
            ...
            onClick={handleAddToCart}
            disabled={loading}
            className="bg-[#08ada8] flex items-center cursor-pointer justify-center flex-1 uppercase font-medium text-[10px] sm:text-[15px] text-white"
          >
          ...
          <div className="md:hidden">
            <Button
              ...
              className="px-3 cursor-pointer flex items-center justify-center bg-gray-200 hover:bg-gray-100 text-gray-600 rounded-full"
            >
              <DotsVerticalIcon />
            </Button>
