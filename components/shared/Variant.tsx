...
//   // Selected index from redux (single source of truth)
//   const selectedIndex = useSelector(
//     (state: RootState) => state.product.selectedVariant[productId] ?? 0
//   );

//   // Local mirror to render immediately; kept in sync with redux
//   const [localSelectedVariant, setLocalSelectedVariant] = useState<number>(selectedIndex);
//   const [selectedSize, setSelectedSize] = useState<string | null>(null);

//   // keep local mirror in sync if redux changes (e.g., other components dispatch)
//   useEffect(() => {
//     setLocalSelectedVariant(selectedIndex);
//   }, [selectedIndex]);

//   const variants = Array.isArray(product.variants) ? product.variants : [];

//   // helper to safely get attribute value by name (case-insensitive)
//   const getAttr = (variant: any, name: string) =>
//     variant?.attributes?.find((a: any) => (a.name || "").toLowerCase() === name.toLowerCase())?.value;

//   // gather unique sizes from variants (case-insensitive treatment)
//   const sizes = useMemo(() => {
...
//   // When user picks a size, find first matching variant and dispatch selection
//   const handleSizePick = (size: string | null) => {
//     if (!size) {
//       setSelectedSize(null);
//       // do not change redux selection when clearing size (optional: you could reset to 0)
//       return;
//     }
...
//     });
//     if (idx >= 0) {
//       // dispatch selected variant to redux (global)
...
//   // When a variant button is clicked, dispatch to redux and sync size state
...
//   // Build a readable label for a variant (excludes size to keep it separate)
//   const buildLabel = (variant: any) => {
...
//   // currentVariant is derived from the redux-selected index (via local mirror)
...
//           <span className="text-3xl font-normal text-red-600">${finalPrice.toLocaleString()}</span>
//           {savings > 0 && <span className="text-sm text-gray-500 line-through">${(finalPrice + savings).toLocaleString()}</span>}
//         </div>
//         {savings > 0 && <div className="text-sm text-green-700">You save: ${savings} ({Math.round((savings / (finalPrice + savings)) * 100)}%)</div>}
//         <div className="text-xs text-gray-600">& FREE Returns</div>
//       </div>

//       {/* Size selector: shown only when there are multiple size values across variants */}
//       {sizes.length > 1 && (
//         <div className="mt-4">
...
//       {/* Variant list: still show all variants so user can pick exact combination (color/other attrs) */}
//       <div className="space-y-4 mt-4">
//         <div>
//           <label className="block text-sm font-medium text-gray-900 mb-2">Configuration:</label>
//           <div className="space-y-2">
//             {variants.length === 0 ? (
//               <div className="text-sm text-gray-600">This product has no variants. Using base product configuration.</div>
//             ) : (
...
//                 // If a size filter is active, visually de-emphasize variants that don't match
...
//                   >
//                     {/* thumbnail */}
//                     <div className="w-14 h-14 flex-shrink-0 rounded overflow-hidden border bg-white">
//                       {img ? (
...
//       {/* Key Features (first 4 attributes from product) */}
