...
//   product?.variants?.[selectedVariantIndex ?? 0] || null;
...
//      try {
//        const {error, success, message} = await toggleAddToWishlist({productId: product?._id as string})
//        if(error) {
//           toast(error.message)
//           return;
//        }else if(success) {
...
//      }
//   }
//   const isWishlisted: boolean = !!user?.wishLists?.some(w => String(w) === String(product?._id));
//      const handleAddToCart = async () => {
//   try {
//     setLoading(true);

//     // Replace these with your actual variant state names if different
   
//     const variant = (selectedVariant ?? null); // e.g. { color: 'red', size: 'M' } or null
...
//       quantity: quantity,
//       // optional fields for local-only usage; the thunk will create its own _id
...
//     // optional small delay for UX / demo
//     await new Promise((resolve) => setTimeout(resolve, 1000));

//     // Dispatch the async action (the thunk expects productId, quantity, variantId/variant if present)
//     await dispatch(addItemAsync(payload) as any);

//     // refresh page / UI if needed
//     router.refresh?.();
...
//           <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-1 border border-gray-300 rounded hover:bg-gray-50" disabled={quantity <= 1}>
//             <Minus className="w-4 h-4" />
//           </button>
//           <span className="px-3 py-1 border border-gray-300 rounded min-w-[50px] text-center">{quantity}</span>
//           <button onClick={() => setQuantity(Math.min(currentVariantStock, quantity + 1))} className="p-1 border border-gray-300 rounded hover:bg-gray-50" disabled={quantity >= currentVariantStock}>
...
//         <Button  disabled={loading || currentVariantStock === 0}
//   onClick={() => handleAddToCart()} className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-medium py-2" >
//            {loading ? <Spinner /> : <><ShoppingCart className="w-4 h-4 mr-2" /> Add to Cart</> } 
//         </Button>

//         <Button className="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-2" disabled={currentVariantStock === 0}>
//           Buy Now
//         </Button>
